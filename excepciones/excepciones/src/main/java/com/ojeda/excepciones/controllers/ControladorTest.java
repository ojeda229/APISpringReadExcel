...
public class ControladorTest {
    ...
    public GenericResponse<UsuariosDTO> registroMasivo( @RequestBody @Valid CargaMasivaRegistry request){
        try{
            return gestionUsuariosServices.registroMasivo(request.getUrlArchivo());

        }catch(Exception e){
            List<String> detalles = Collections.singletonList(e.toString());
